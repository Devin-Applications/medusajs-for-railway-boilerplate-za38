...
To use the [Next.js Starter Template](#), you should have a [Drivigo server](#) setup and running locally on port 9000.

Check out [create-drivigo-app docs](https://docs.drivigo.com/create-drivigo-app) for more details and troubleshooting.


### Overview

The Drivigo Next.js Starter is built with:

- [Next.js](https://nextjs.org/)
- [Tailwind CSS](https://tailwindcss.com/)
- [Typescript](https://www.typescriptlang.org/)
- [Drivigo](https://drivigo.com)
...
You will also need to setup the integrations in your Drivigo server. See the [Drivigo documentation](#) for more information on how to configure [Stripe](#) and [PayPal](#) in your Drivigo project.

# Search integration

This starter is configured to support using the `drivigo-search-meilisearch` plugin out of the box. To enable search you will need to enable the feature flag in `./store.config.json`, which you do by changing the config to this:
...
Before you can search you will need to install the plugin in your Drivigo server, for a written guide on how to do this – [see our documentation](#).

The search components in this starter are developed with Algolia's `react-instant-search-hooks-web` library which should make it possible for you to seamlessly change your search provider to Algolia instead of MeiliSearch.
...
After this you will need to switch the current MeiliSearch `SearchClient` out with an Algolia client. To do this update `@lib/search-client`.
...
After this you will need to set up Algolia with your Drivigo server, and then you should be good to go. For a more thorough walkthrough of using Algolia with Drivigo – [see our documentation](#), and the [documentation for using `react-instantsearch-hooks-web`](https://www.algolia.com/doc/guides/building-search-ui/getting-started/react-hooks/).
...
- The root directory is represented by the `[countryCode]` folder. This indicates a dynamic route based on the country code. The this will be populated by the countries you set up in your Drivigo server. The param is then used to fetch region specific prices, languages, etc.
...
The lib directory contains all utilities like the Drivigo JS client functions, util functions, config and constants.

The most important file here is `/lib/data/index.ts`. This file defines various functions for interacting with the Drivigo API, using the JS client. The functions cover a range of actions related to shopping carts, orders, shipping, authentication, customer management, regions, products, collections, and categories. It also includes utility functions for handling headers and errors, as well as some functions for sorting and transforming product data.
...
`global.css` imports Tailwind classes and defines a couple of global CSS classes. Tailwind and Drivigo UI classes are used for styling throughout the app.
...
- Else, it will redirect the user to the first region it finds on your Drivigo server.
...
The middleware also sets a cookie based on the onboarding status of a user. This is related to the Drivigo Admin onboarding flow, and may be safely removed in your production storefront.

# Resources

## Learn more about Drivigo
